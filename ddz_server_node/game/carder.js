...
module.exports = function(){
    ...
    const compareOne = function(cardA,cardB){
        ...
        var valueA = 0;
        console.log(cardA[0]);
        console.log(cardA[0].card_data);
        const {value=''} = cardA[0].card_data;
        if(value==''){
        ...
        var valueB = 0;
        const {value:value1=''} = cardB[0].card_data;
        if(value1==''){
    ...
    const comparePlanWithSing = function(cardA,cardB){
        ...
        for(var i=0;i<cardA.length;i++){
            console.log('caedA',cardA);
            console.log('cardA.card_data',cardA.card_data);
            if(map.hasOwnProperty(cardA[i].card_data.value)){
                lista.push(cardA[i])
            }else{
                map[cardA[i].card_data.value] = 1
            }
        }

        for(var i=0;i<cardB.length;i++){
            if(map.hasOwnProperty(cardB[i].card_data.value)){
                listb.push(cardB[i])
            }else{
                map[cardB[i].card_data.value] = 1
            }
        }
    ...
    const comparePlanWithTow = function(cardA,cardB){
        ...
        for (var i = 0; i < cardA.length; i++) {
            if (mapA.hasOwnProperty(cardA[i].card_data.value)) {
                mapA[cardA[i].card_data.value].push(cardA[i]);  //修改bug 之前cardA没有指定哪一项
            } else {
    ...
    const compare = function(cardA,cardB,current_card_value){
        ...
        switch(current_card_value.name){
            ...
            case CardsValue.three.name:
                ...
                break
            case CardsValue.threeWithOne.name:  //新增 三带一比较
            ...
            case  CardsValue.doubleScroll.name:
                ...
                break               
            default:
                console.log('card_last_value.name',current_card_value.name);
    ...
    that.compareWithCard = function(last_cards,current_cards){
        ...
        card_last_value = getCardValue(last_cards)  //获取当前牌型的value
