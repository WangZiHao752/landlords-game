   ...
   that.removePushCards = function(remve_cards){
        ...
        for(var i=0;i<remve_cards.length;i++){
            ...
            for(var j=0;j<that._cards.length;j++){
                //console.log('rcard.cardid',rcard.cardid);
                //console.log('that._cards[j].cardid',that._cards[j].index); //that._cards[j].cardid undefined
                    ...
                    case "enterroom_req": //获取房间的用户列表
                     ...
                     case "player_start_notify": //客户端:房主发送开始游戏消息
                      ...
                      case "player_rob_notify":  //客户端发送抢地主消息
                           ...
                           break 
                       case "chu_bu_card_req":   //客户端发送不出牌消息
                            if(that._room){
                       ...
                       case "chu_card_req":
                            if(that._room){
                                ...
                                that._room.playerChuCard(that,data,function(err,result){
                                    if(err){
                                    console.log('playerChuCard cb err-RESULT',JSON.stringify(result)); 
